...
.kpi-value {
    font-size: 1.6rem; font-weight: 700; color: #1C2B4A;
    font-family: 'IBM Plex Mono', monospace;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.kpi-label { font-size: 0.72rem; color: #64748B; margin-top: 6px; text-transform: uppercase; letter-spacing: 0.5px; }
...
if page == "Vue Globale":
    ...
    c1.markdown(kpi(n_cl,            "Clients surveilles","navy","ti-users"),         unsafe_allow_html=True)
    c2.markdown(kpi(f"{conso_m:.1f} kWh","Conso. moyenne","blue","ti-bolt"),          unsafe_allow_html=True)
    ...
    c5.markdown(kpi(n_crit+n_elev,   "Critique / Eleve","red","ti-shield-exclamation"), unsafe_allow_html=True)
    ...
    fig4.update_layout(
        **layout_base(380),
        xaxis=dict(title="Jour de la semaine", side="bottom", tickfont=dict(size=12)),
        yaxis=dict(title="Heure de la journee", tickmode="linear", dtick=2, autorange="reversed"),
...
# ════════════════════════════════════
    ...
    c2.markdown(kpi(info["type_client"].capitalize(),"Type de client","blue","ti-building"), unsafe_allow_html=True)
    ...
    c4.markdown(f"""<div class='kpi-card red'>
        <div class='kpi-icon'><i class='ti ti-shield'></i></div>
...
# ════════════════════════════════════
    ...
    c1,c2,c3,c4 = st.columns(4)
    c1.markdown(kpi(dist["Critique"],"Critique","red","ti-alert-circle"),    unsafe_allow_html=True)
    c2.markdown(kpi(dist["Eleve"],   "Eleve","amber","ti-alert-triangle"),   unsafe_allow_html=True)
    c3.markdown(kpi(dist["Modere"],  "Modere","blue","ti-info-circle"),      unsafe_allow_html=True)
    c4.markdown(kpi(dist["Faible"],  "Faible","green","ti-circle-check"),    unsafe_allow_html=True)
...
# ════════════════════════════════════
    ...
    mc4.markdown(kpi("4.23 kWh", "LSTM — MAE (meilleur)","green","ti-trophy"), unsafe_allow_html=True)
...
# ════════════════════════════════════
    ...
    ac1.markdown(kpi(f"{int(len(df)*0.0462):,}","Anomalies IF (4.62%)","amber","ti-alert-triangle"), unsafe_allow_html=True)
    ac2.markdown(kpi("1,092","Anomalies LSTM AE (3.12%)","blue","ti-cpu"),                            unsafe_allow_html=True)
    ac3.markdown(kpi("8","Consensus IF + AE","red","ti-shield-exclamation"),                           unsafe_allow_html=True)
