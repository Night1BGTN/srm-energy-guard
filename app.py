...
def generate_rapport(df, risk_df):
    ...
    story.append(HRFlowable(width="100%", thickness=1.5, color=NAVY, spaceAfter=8))
    story.append(Paragraph(
        "Quatre modeles evalues sur le dataset UCI ElectricityLoadDiagrams. "
...
# ════════════════════════════════════
    ...
    # ── SECTION EXPLICABILITE AUTOMATIQUE ──
    section("Analyse Explicative — Interpretation automatique des resultats")

    jours_fr = {0:"Lundi",1:"Mardi",2:"Mercredi",3:"Jeudi",4:"Vendredi",5:"Samedi",6:"Dimanche"}

    # Calculs dynamiques sur toutes les donnees du client
    anom_all     = detect_anom(df_c)
    anom_pts     = df_c[anom_all==1].copy()
    ...
    n_anom_c     = len(anom_pts)
    score_c      = info["score"]
    niveau_c     = info["niveau"]
    type_c       = info["type_client"]

    # Couleur selon niveau
    color_niv = {"Critique":"#DC2626","Eleve":"#D97706","Modere":"#2563EB","Faible":"#16A34A"}
    bg_niv    = {"Critique":"#FEE2E2","Eleve":"#FEF3C7","Modere":"#EFF6FF","Faible":"#F0FDF4"}
    c_niv     = color_niv.get(niveau_c,"#64748B")
    b_niv     = bg_niv.get(niveau_c,"#F7F8FA")

    # ── Bloc 1 : Pourquoi ce niveau de risque ──
    ex1,ex2 = st.columns(2)

    with ex1:
        # Raisons du score
        raisons = []
        if conso_std / (conso_moy+1e-9) > 0.5:
            raisons.append(f"Variabilite elevee : ecart-type = <b>{conso_std:.1f} kWh</b> ({conso_std/conso_moy*100:.0f}% de la moyenne)")
        else:
            raisons.append(f"Variabilite normale : ecart-type = <b>{conso_std:.1f} kWh</b>")

        if conso_moy > df["consommation_kwh"].mean() * 2:
            raisons.append(f"Consommation <b>tres elevee</b> : {conso_moy:.1f} kWh ({conso_moy/df['consommation_kwh'].mean():.1f}x la moyenne globale)")
        elif conso_moy > df["consommation_kwh"].mean():
            raisons.append(f"Consommation <b>au-dessus</b> de la moyenne globale ({conso_moy:.1f} vs {df['consommation_kwh'].mean():.1f} kWh)")
        else:
            raisons.append(f"Consommation <b>en dessous</b> de la moyenne globale ({conso_moy:.1f} vs {df['consommation_kwh'].mean():.1f} kWh)")

        if type_c == "industriel":
            raisons.append("Type <b>industriel</b> : facteur de risque majore (+20% dans le score)")
        elif type_c == "commercial":
            raisons.append("Type <b>commercial</b> : profil de consommation variable")
        else:
            raisons.append("Type <b>residentiel</b> : consommation attendue stable")

        raisons.append(f"Score final : <b>{score_c:.1f}%</b> — classement <b>{niveau_c}</b>")

        raisons_html = "".join([f"<li style='margin:6px 0'>{r}</li>" for r in raisons])
        st.markdown(f"""
        <div style='background:{b_niv};border:1px solid {c_niv};border-left:4px solid {c_niv};
        border-radius:8px;padding:16px 20px;'>
            <div style='font-size:0.95rem;font-weight:700;color:{c_niv};margin-bottom:10px'>
                Pourquoi niveau {niveau_c} ?
            </div>
            <ul style='margin:0;padding-left:18px;font-size:0.88rem;color:#1A1D23;line-height:1.6'>
                {raisons_html}
            </ul>
        </div>""", unsafe_allow_html=True)
    ...
    with ex2:
        # Anomalies detectees
        if n_anom_c == 0:
            anom_html = "<p style='color:#16A34A;font-weight:600'>Aucune anomalie detectee sur toute la periode.</p>"
        else:
            lignes = []
            # Top 5 anomalies les plus importantes
            anom_sorted = anom_pts.copy()
            anom_sorted["ecart"] = abs(anom_sorted["consommation_kwh"] - anom_sorted["conso_moy_7j"])
            top_anom = anom_sorted.nlargest(min(5, n_anom_c), "ecart")
            for _,row in top_anom.iterrows():
                date_str  = row["timestamp"].strftime("%d/%m/%Y")
                heure_str = row["timestamp"].strftime("%Hh")
                jour_str  = jours_fr.get(row["timestamp"].weekday(), "")
                conso_val = row["consommation_kwh"]
                ecart_val = row["consommation_kwh"] - row["conso_moy_7j"]
                signe     = "+" if ecart_val > 0 else ""
                ratio     = conso_val / (conso_moy+1e-9)
                lignes.append(
                    f"<li style='margin:6px 0'>"
                    f"<b>{date_str} a {heure_str}</b> ({jour_str})<br>"
                    f"<span style='color:#DC2626'>{conso_val:.1f} kWh</span> "
                    f"({signe}{ecart_val:.1f} kWh vs moy 7j, soit <b>{ratio:.1f}x</b> la moyenne client)"
                    f"</li>")
            anom_html = f"""
            <div style='font-size:0.82rem;color:#64748B;margin-bottom:6px'>
                Total sur toute la periode : <b style='color:#DC2626'>{n_anom_c} anomalie(s)</b>
                — Top {min(5,n_anom_c)} les plus importantes :
            </div>
            <ul style='margin:0;padding-left:18px;font-size:0.85rem;color:#1A1D23;line-height:1.6'>
                {"".join(lignes)}
            </ul>"""

        st.markdown(f"""
        <div style='background:#FFF7ED;border:1px solid #D97706;border-left:4px solid #D97706;
        border-radius:8px;padding:16px 20px;'>
            <div style='font-size:0.95rem;font-weight:700;color:#D97706;margin-bottom:10px'>
                Anomalies detectees — {client_sel}
            </div>
            {anom_html}
        </div>""", unsafe_allow_html=True)

    # ── Bloc 2 : Statistiques cles ──
    st.markdown("<br>", unsafe_allow_html=True)
    s1,s2,s3,s4 = st.columns(4)
    s1.markdown(kpi(f"{n_anom_c}", "Anomalies totales","red","ti-alert-triangle"),          unsafe_allow_html=True)
    s2.markdown(kpi(f"{conso_max:.1f} kWh","Pic maximum","amber","ti-arrow-up"),             unsafe_allow_html=True)
    s3.markdown(kpi(f"{conso_min:.1f} kWh","Minimum","blue","ti-arrow-down"),                unsafe_allow_html=True)
    s4.markdown(kpi(f"{conso_std:.1f} kWh","Ecart-type","navy","ti-chart-bar"),              unsafe_allow_html=True)
