...
def generate_rapport(df, risk_df):
    """Genere le rapport PDF simple et professionnel"""
    ...
    s_title = ParagraphStyle("s_title", parent=styles["Title"],
        fontSize=20, textColor=DARK, alignment=TA_LEFT,
        spaceAfter=4, fontName="Helvetica-Bold")
    s_sub = ParagraphStyle("s_sub", parent=styles["Normal"],
        fontSize=10, textColor=GRAY, alignment=TA_LEFT, spaceAfter=2)
    s_period = ParagraphStyle("s_period", parent=styles["Normal"],
        fontSize=9, textColor=GRAY, alignment=TA_LEFT, spaceAfter=0)
    ...
    s_body = ParagraphStyle("s_body", parent=styles["Normal"],
        fontSize=9, textColor=DARK, leading=14, spaceAfter=8)
    ...
    # ── TITRE ──
    story.append(Paragraph("SRM Smart Energy Guard", s_title))
    story.append(Paragraph("Rapport de Surveillance Energetique", s_sub))
    story.append(Paragraph(f"Periode : {d_min} — {d_max}", s_period))
    story.append(Spacer(1, 0.2*cm))
    story.append(HRFlowable(width="100%", thickness=1.5, color=DARK, spaceAfter=16))

    # ── 1. RESUME EXECUTIF ──
    story.append(Paragraph("1. Indicateurs de Surveillance", s_h1))
    story.append(HRFlowable(width="100%", thickness=0.5, color=LINE, spaceAfter=10))

    kpi_data = [
        ["Indicateur", "Valeur"],
        ["Clients surveilles",  f"{n_cl}"],
        ["Consommation moyenne", f"{conso_m:.2f} kWh / mesure"],
        ["Consommation totale",  f"{conso_tot/1e6:.2f} GWh"],
        ["Anomalies IF",         f"{n_anom:,}  (4.62%)"],
        ["Anomalies LSTM AE",    f"{n_anom_ae:,}  (3.12%)"],
        ["Consensus IF + AE",    "8"],
        ["Clients a risque",     f"{n_crit + n_elev}  (Critique + Eleve)"],
        ["MAE LSTM",             "4.35 kWh"],
    ]
    t = Table(kpi_data, colWidths=[8*cm, 9*cm])
    ...
    story.append(Paragraph(
        f"Scoring multicritere : ecart de consommation (50%), variabilite (30%), type de client (20%).",
        s_body))

    risk_sum = [
        ["Niveau", "Clients", "Pourcentage"],
        ["Critique", str(n_crit), f"{n_crit/n_cl*100:.1f}%"],
        ["Eleve",    str(n_elev), f"{n_elev/n_cl*100:.1f}%"],
        ["Modere",   str(n_mod),  f"{n_mod/n_cl*100:.1f}%"],
        ["Faible",   str(n_faib), f"{n_faib/n_cl*100:.1f}%"],
    ]
    rt = Table(risk_sum, colWidths=[6*cm, 5*cm, 6*cm])
    ...
    # Top 10 clients
    story.append(Paragraph("Top 10 clients a risque eleve :", s_h1))
    story.append(HRFlowable(width="100%", thickness=0.5, color=LINE, spaceAfter=8))
    top10 = risk_df.nlargest(10,"score")[["client_id","type_client","conso_moy","conso_std","score","niveau"]].copy()
    t10d  = [["Client ID", "Type", "Conso Moy (kWh)", "Ecart-type (kWh)", "Score (%)", "Niveau"]]
    for _,row in top10.iterrows():
        t10d.append([
            row["client_id"],
            row["type_client"].capitalize(),
            f"{row['conso_moy']:.1f}",
            f"{row['conso_std']:.1f}",
            f"{row['score']:.1f}%",
            row["niveau"]
        ])
    t10 = Table(t10d, colWidths=[3*cm, 3*cm, 3.5*cm, 3.5*cm, 2.5*cm, 2.5*cm])
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=LINE, spaceAfter=8))
    story.append(Paragraph(
        "Comparaison de 4 modeles sur SRM-001 — split chronologique 80% train / 20% test "
        "(35,065 mesures horaires, 2011-2014).", s_body))

    ml_data = [
        ["Modele", "MAE (kWh)", "RMSE (kWh)", "Gain vs Prophet (%)"],
        ["Prophet",  "13.56", "16.19", "—"],
        ["GRU",      "6.95",  "9.55",  "+48.8%"],
        ["CNN-LSTM", "9.27",  "11.92", "+31.6%"],
        ["LSTM",     "4.35",  "5.83",  "+67.9%"],
    ]
    mlt = Table(ml_data, colWidths=[4*cm, 4*cm, 4*cm, 5*cm])
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=LINE, spaceAfter=8))
    anom_data = [
        ["Methode", "Anomalies", "Taux"],
        ["Isolation Forest", f"{n_anom:,}", "4.62%"],
        ["LSTM Autoencoder", f"{n_anom_ae:,}", "3.19%"],
        ["Consensus IF + AE", "8", "—"],
    ]
    at = Table(anom_data, colWidths=[6*cm, 5*cm, 6*cm])
...
def generate_rapport_client(df, risk_df, client_id):
    """Genere un rapport PDF individuel pour un client"""
    ...
    styles = getSampleStyleSheet()
    s_title = ParagraphStyle("ct", parent=styles["Title"],
        fontSize=20, textColor=DARK, alignment=TA_LEFT,
        spaceAfter=4, fontName="Helvetica-Bold")
    s_sub = ParagraphStyle("cs", parent=styles["Normal"],
        fontSize=10, textColor=GRAY, alignment=TA_LEFT, spaceAfter=2)
    s_period = ParagraphStyle("cp", parent=styles["Normal"],
        fontSize=9, textColor=GRAY, alignment=TA_LEFT, spaceAfter=0)
    s_h1 = ParagraphStyle("ch1", parent=styles["Heading1"],
        fontSize=12, textColor=DARK, spaceBefore=16, spaceAfter=8,
        fontName="Helvetica-Bold")
    s_body = ParagraphStyle("cb", parent=styles["Normal"],
        fontSize=9, textColor=DARK, leading=14, spaceAfter=6)

    df_c   = df[df["client_id"] == client_id].sort_values("timestamp")
    info   = risk_df[risk_df["client_id"] == client_id].iloc[0]
    d_min  = df_c["timestamp"].min().strftime("%d/%m/%Y")
    d_max  = df_c["timestamp"].max().strftime("%d/%m/%Y")
    n_mes  = len(df_c)
    conso_min  = df_c["consommation_kwh"].min()
    conso_max  = df_c["consommation_kwh"].max()
    conso_med  = df_c["consommation_kwh"].median()
    conso_moy  = df_c["consommation_kwh"].mean()
    conso_std  = df_c["consommation_kwh"].std()
    z = (df_c["consommation_kwh"] - df_c["conso_moy_7j"]) / (df_c["std_24h"] + 1e-9)
    anom_mask = z.abs() > 3
    n_anom = anom_mask.sum()
    anom_pct = n_anom / n_mes * 100 if n_mes > 0 else 0

    story = []

    story.append(Paragraph(f"Rapport Client — {client_id}", s_title))
    story.append(Paragraph(f"Type : {info['type_client'].capitalize()}  |  Niveau de risque : {info['niveau']}", s_sub))
    story.append(Paragraph(f"Periode : {d_min} — {d_max}", s_period))
    story.append(Spacer(1, 0.2*cm))
    story.append(HRFlowable(width="100%", thickness=1.5, color=DARK, spaceAfter=16))
    ...
    fiche = [
        ["Indicateur", "Valeur"],
        ["Client ID",           client_id],
        ["Type de client",      info["type_client"].capitalize()],
        ["Niveau de risque",    info["niveau"]],
        ["Score de risque",     f"{info['score']:.1f}%"],
        ["Consommation moyenne",f"{conso_moy:.2f} kWh"],
        ["Ecart-type",          f"{conso_std:.2f} kWh"],
        ["Consommation min",    f"{conso_min:.2f} kWh"],
        ["Consommation max",    f"{conso_max:.2f} kWh"],
        ["Mediane",             f"{conso_med:.2f} kWh"],
        ["Total mesures",       f"{n_mes:,}"],
    ]
    tf = Table(fiche, colWidths=[8*cm, 9*cm])
    ...
    anom_resume = [
        ["Indicateur", "Valeur"],
        ["Total mesures analysees", f"{n_mes:,}"],
        ["Anomalies detectees",     f"{n_anom}"],
        ["Taux d anomalies",        f"{anom_pct:.2f}%"],
        ["Methode de detection",    "Z-score > 3"],
    ]
    ta = Table(anom_resume, colWidths=[8*cm, 9*cm])
    ...
    anom_rows = df_c[anom_mask].nlargest(10, "consommation_kwh")[["timestamp","consommation_kwh","conso_moy_7j"]].copy()
    top_anom_data = [["Date / Heure", "Consommation (kWh)", "Moyenne 7j (kWh)"]]
    for _, row in anom_rows.iterrows():
        top_anom_data.append([
            row["timestamp"].strftime("%d/%m/%Y %Hh"),
            f"{row['consommation_kwh']:.2f}",
            f"{row['conso_moy_7j']:.2f}",
        ])

    anom_block = [
        Paragraph("Anomalies Detectees", s_h1),
        HRFlowable(width="100%", thickness=0.5, color=LINE, spaceAfter=8),
        ta, Spacer(1, 0.3*cm),
    ]
    if len(top_anom_data) > 1:
        t_anom = Table(top_anom_data, colWidths=[6*cm, 5.5*cm, 5.5*cm])
        ...
        anom_block += [Paragraph("Top anomalies les plus importantes :", s_body), Spacer(1, 0.2*cm), t_anom]
    else:
        anom_block.append(Paragraph("Aucune anomalie significative detectee.", s_body))
    anom_block.append(Spacer(1, 0.5*cm))
    story.append(KeepTogether(anom_block))

    prof = df_c.groupby("heure")["consommation_kwh"].agg(["mean","min","max"]).reset_index()
    prof_data = [["Heure", "Conso Moyenne (kWh)", "Minimum (kWh)", "Maximum (kWh)"]]
    for _, row in prof.iterrows():
        prof_data.append([f"{int(row['heure']):02d}h", f"{row['mean']:.2f}", f"{row['min']:.2f}", f"{row['max']:.2f}"])
    tp = Table(prof_data, colWidths=[3.5*cm, 5*cm, 4*cm, 4.5*cm])
    ...
    story.append(KeepTogether([
        Paragraph("Profil Horaire Moyen", s_h1),
        HRFlowable(width="100%", thickness=0.5, color=LINE, spaceAfter=8),
        Paragraph("Consommation moyenne, minimum et maximum par heure de la journee.", s_body),
        Spacer(1, 0.2*cm), tp,
    ]))
...
.kpi-card.yellow { border-top-color: #EAB308; } .kpi-card.yellow .kpi-value { color: #EAB308; }
.section-title {
...
RISK_COLORS = {"Faible":"#16A34A","Modere":"#EAB308","Eleve":"#D97706","Critique":"#DC2626"}
...
@st.cache_data(show_spinner=False)
def get_daily_data(_df):
    df2 = _df.copy()
    df2["date_only"] = df2["timestamp"].dt.date
    daily = df2.groupby(["date_only","type_client"])["consommation_kwh"].mean().reset_index()
    daily["date_only"] = pd.to_datetime(daily["date_only"])
    daily["type_client"] = daily["type_client"].astype(str)
    return daily

@st.cache_data(show_spinner=False)
def get_hourly_type(_df):
    hourly = _df.groupby(["heure","type_client"])["consommation_kwh"].mean().reset_index()
    hourly["type_client"] = hourly["type_client"].astype(str)
    return hourly

@st.cache_data(show_spinner=False)
def get_heatmap_data(_df):
    pivot = _df.groupby(["heure","jour_semaine"])["consommation_kwh"].mean().unstack()
    jours = ["Lun","Mar","Mer","Jeu","Ven","Sam","Dim"]
    pivot.columns = [jours[c] if c < len(jours) else str(c) for c in pivot.columns]
    return pivot

@st.cache_data(show_spinner=False)
def get_monthly_data(_df, year):
    df2 = _df.copy()
    df2["year"]  = df2["timestamp"].dt.year
    df2["month"] = df2["timestamp"].dt.month
    filtered = df2[df2["year"] == year]
    monthly = filtered.groupby(["month","type_client"])["consommation_kwh"].mean().reset_index()
    monthly["type_client"] = monthly["type_client"].astype(str)
    return monthly

@st.cache_data(show_spinner=False)
def get_type_count(_df):
    tc_cnt = _df.groupby("type_client")["client_id"].nunique().reset_index()
...
ML = {
    ...
    "GRU":      {"MAE": 6.95,  "RMSE": 9.55,  "Gain": 48.8, "color": "#D97706"},
    "CNN-LSTM": {"MAE": 9.27,  "RMSE": 11.92, "Gain": 31.6, "color": "#2563EB"},
    "LSTM":     {"MAE": 4.35,  "RMSE": 5.83,  "Gain": 67.9, "color": "#16A34A"},
}
n_anom_ae = 1117
n_anom_ae_pct = 3.19  # taux reel calcule sur sequences 24h lors du training

# ── Bouton rapport PDF dans sidebar (apres calcul risk_df) ──
with st.sidebar:
    st.markdown("<hr style='border-color:#2D4A7A;margin:8px 0'>", unsafe_allow_html=True)
    if st.button("Generer le Rapport PDF", use_container_width=True):
        with st.spinner("Generation du rapport..."):
            pdf_bytes = generate_rapport(df, risk_df)
        st.download_button(
...
if page == "Vue Globale":
    ...
    c4.markdown(kpi("4.35 kWh",              "MAE LSTM","green","ti-brain"),                        unsafe_allow_html=True)
    c5.markdown(kpi(f"{n_crit+n_elev}",      "Critique+Eleve","red","ti-shield-exclamation"),       unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    section("Consommation moyenne par type de client (2011-2014)")
    ...
    # Profil horaire — 3 graphes cote a cote sous le donut
    section("Profil horaire moyen par type")
    PH_COLORS = ["#93C5FD", "#FCD34D", "#FCA5A5"]  # bleu clair, jaune clair, rouge clair
    hourly = get_hourly_type(df)
    type_order = sorted(hourly["type_client"].unique())
    ph1, ph2, ph3 = st.columns(3)
    cols_ph = [ph1, ph2, ph3]
    for i, tc in enumerate(type_order):
        h = hourly[hourly["type_client"] == tc]
        fig_ph = go.Figure()
        fig_ph.add_trace(go.Scatter(
            x=h["heure"], y=h["consommation_kwh"],
            mode="lines+markers", name=tc.capitalize(),
            line=dict(color=PH_COLORS[i % len(PH_COLORS)], width=2.5),
            marker=dict(size=5, color=PH_COLORS[i % len(PH_COLORS)]),
            hovertemplate="Heure %{x}h: %{y:.1f} kWh<extra></extra>"))
        fig_ph.update_layout(
            height=220,
            paper_bgcolor=BG, plot_bgcolor=BG, font=FONT,
            title=dict(text=tc.capitalize(), font=dict(size=12, color="#1C2B4A"), x=0.5),
            xaxis=dict(tickmode="linear", dtick=4, title="Heure",
        ...
        cols_ph[i].plotly_chart(fig_ph, width='stretch')

    # ── Graphe mensuel par année ──
    section("Consommation mensuelle par type — Analyse annuelle")
    ...
    monthly_data = get_monthly_data(df, year_sel)
    MOIS = ["Jan","Fev","Mar","Avr","Mai","Jun","Jul","Aou","Sep","Oct","Nov","Dec"]
    PH_COLORS = ["#93C5FD", "#FCD34D", "#FCA5A5"]
    fig_month = go.Figure()
    for i, tc in enumerate(sorted(monthly_data["type_client"].unique())):
        m = monthly_data[monthly_data["type_client"] == tc].sort_values("month")
        fig_month.add_trace(go.Bar(
            x=[MOIS[mo-1] for mo in m["month"]],
            y=m["consommation_kwh"],
            name=tc.capitalize(),
            marker_color=PH_COLORS[i % len(PH_COLORS)],
            hovertemplate=f"{tc.capitalize()}<br>%{{x}} {year_sel}: %{{y:.1f}} kWh<extra></extra>"
        ))
    fig_month.update_layout(
        **layout_base(320),
        barmode="group",
        xaxis=dict(title="Mois", showgrid=False, categoryorder="array",
                   categoryarray=MOIS),
        yaxis=dict(title="Consommation moyenne (kWh)", gridcolor="#F1F5F9",
                   rangemode="tozero"),
        legend=dict(orientation="h", y=1.08),
        template="plotly_white",
...
# ════════════════════════════════════
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    section("Serie temporelle — derniere periode disponible")
    ...
    with cp2:
        ...
        q25 = df_c["consommation_kwh"].quantile(0.25)
        q75 = df_c["consommation_kwh"].quantile(0.75)
        st.caption(f"La majorite des mesures se situe entre {q25:.0f} et {q75:.0f} kWh pour ce client.")
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    section("Rapport individuel du client")
    if st.button(f"Telecharger le rapport de {client_sel}", use_container_width=True):
        with st.spinner("Generation du rapport client..."):
            pdf_client = generate_rapport_client(df, risk_df, client_sel)
        st.download_button(
            label=f"Rapport PDF — {client_sel}",
            data=pdf_client,
            file_name=f"rapport_{client_sel}_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
            mime="application/pdf",
            use_container_width=True,
        )
...
# ════════════════════════════════════
    ...
    header("Carte de Risque — 99 Clients",
    ...
    c3.markdown(kpi(f"{dist['Modere']}", "Modere",  "amber","ti-info-circle"),      unsafe_allow_html=True)
    ...
    top5_ids = risk_df.nlargest(5,"score")["client_id"].tolist()
    fig_sc = go.Figure()
    for niv,color in RISK_COLORS.items():
        sub = risk_df[risk_df["niveau"]==niv]
        fig_sc.add_trace(go.Scatter(
            x=sub["conso_moy"], y=sub["score"], mode="markers+text", name=niv,
            marker=dict(color=color,
                        size=(sub["conso_std"]/risk_df["conso_std"].max()*20+6).clip(6,26),
                        opacity=0.8, line=dict(width=0.5, color="white")),
            text=[cid if cid in top5_ids else "" for cid in sub["client_id"]],
            textposition="top center",
            textfont=dict(size=9, color=color),
            customdata=sub["client_id"],
            hovertemplate="<b>%{customdata}</b><br>Conso moy: %{x:.1f} kWh<br>Score: %{y:.1f}%<extra></extra>"))
    ...
    st.dataframe(disp.reset_index(drop=True), width='stretch', height=380, hide_index=True)
...
# ════════════════════════════════════
    ...
    mc2.markdown(kpi("6.95 kWh", "GRU — MAE","amber","ti-chart-line"),        unsafe_allow_html=True)
    mc3.markdown(kpi("9.27 kWh", "CNN-LSTM — MAE","blue","ti-chart-line"),    unsafe_allow_html=True)
    mc4.markdown(kpi("4.35 kWh", "LSTM — MAE","green","ti-trophy"),           unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    models_df = pd.DataFrame([
        {"Modele":"Prophet",  "MAE":13.56,"RMSE":16.19,"Gain":0.0},
        {"Modele":"GRU",      "MAE":6.95, "RMSE":9.55, "Gain":48.8},
        {"Modele":"CNN-LSTM", "MAE":9.27, "RMSE":11.92,"Gain":31.6},
        {"Modele":"LSTM",     "MAE":4.35, "RMSE":5.83, "Gain":67.9},
    ])
    ...
    st.caption("Note : Les modeles de prevision ont ete entraines sur SRM-001 (split 80/20 chronologique, 35,065 mesures). La simulation illustre le comportement des modeles avec les erreurs reelles observees lors du test (MAE LSTM = 4.35 kWh).")
    client_p = "SRM-001"
    df_p = df[df["client_id"]==client_p].sort_values("timestamp").tail(168)
    # Seed base sur le client : bruit reproductible mais different par client
    seed_val = sum(ord(c) for c in client_p)
    np.random.seed(seed_val)
    real_vals    = df_p["consommation_kwh"].values
    pred_lstm    = np.clip(real_vals + np.random.normal(0, 4.35, len(df_p)),  0, None)
    ...
    fig_sim.add_trace(go.Scatter(
        ...
        mode="lines", name="LSTM (MAE=4.35 kWh)",
