...
<style>
    ...
    /* Sidebar */
    .css-1d391kg { background-color: #131722 !important; }
...
def compute_risk_score(df: pd.DataFrame) -> pd.DataFrame:
    """Calcul du score de risque par client (0-100%)."""
    ...
    def niveau(s):
        ...
        elif s >= 6:  return "Élevé"
        elif s >= 3:  return "Modéré"
        else:         return "Faible"
...
def detect_anomalies_if(df: pd.DataFrame) -> pd.Series:
    """Détection d'anomalies simplifiée (z-score) pour affichage."""
...
with st.sidebar:
    st.markdown("## ⚡ SRM Energy Guard")
    st.markdown("---")

    uploaded = st.file_uploader(
        "📂 Charger srm_100clients.parquet",
        type=["parquet"],
        help="Glissez-déposez votre fichier parquet ici",
    )
    ...
    page = st.radio(
        "Navigation",
        ["🏠 Vue Globale", "👤 Surveillance Client", "⚠️ Score de Risque",
         "📈 Prédictions", "🔔 Alertes"],
    )
    st.markdown("---")
...
# ─────────────────────────────────────────────
# PAGE 0 — ACCUEIL SI PAS DE DONNÉES
# ─────────────────────────────────────────────
...
if not DATA_LOADED:
    ...
    <div style='text-align:center; padding:80px 20px;'>
        ...
        <p style='color:#8892B0; font-size:1.1rem; max-width:500px; margin:0 auto 32px;'>
            Système intelligent de surveillance énergétique — PFE Licence
        </p>
...
# ─────────────────────────────────────────────
# DONNÉES CHARGÉES — CALCULS COMMUNS
# ─────────────────────────────────────────────

df = df_raw.copy()
risk_df = compute_risk_score(df)
n_clients = df["client_id"].nunique()
n_anomalies_est = int(len(df) * 0.0462)   # taux IF global
...
if page == "🏠 Vue Globale":
    st.markdown("## 🏠 Vue Globale du Réseau")

    # KPIs
    col1, col2, col3, col4, col5 = st.columns(5)
    kpis = [
        (n_clients, "Clients surveillés"),
        (f"{df['consommation_kwh'].mean():.1f} kWh", "Conso. moyenne"),
        (f"{n_anomalies_est:,}", "Anomalies détectées (IF)"),
        ("4.23 kWh", "MAE prévision LSTM"),
    ...
    st.markdown("<div class='section-title'>Consommation journalière par type de client</div>",
                unsafe_allow_html=True)
    ...
    fig1 = px.line(
        daily_agg, x="date", y="consommation_kwh", color="type_client",
        color_discrete_map=COLORS, labels={"consommation_kwh": "kWh", "date": "Date", "type_client": "Type"},
        **PLOTLY_THEME,
    )
    ...
    st.markdown("<div class='section-title'>Heatmap — Consommation par heure × jour</div>",
                unsafe_allow_html=True)
    ...
    fig4.update_layout(height=300, xaxis_title="Jour", yaxis_title="Heure",
                       **PLOTLY_THEME)
...
# ══════════════════════════════════════════════
    ...
    # En-tête client
    badge_map = {"Critique": "critique", "Élevé": "eleve", "Modéré": "modere", "Faible": "faible"}
    ...
    # Série temporelle (derniers 30 jours)
    st.markdown("<div class='section-title'>Série temporelle — 30 derniers jours</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
    ...
    st.markdown("## ⚠️ Carte de Risque — 100 Clients")

    # Distribution niveaux
    dist = risk_df["niveau_risque"].value_counts().reindex(
        ["Critique", "Élevé", "Modéré", "Faible"], fill_value=0
    )
    col1, col2, col3, col4 = st.columns(4)
    icons = {"Critique": "🔴", "Élevé": "🟠", "Modéré": "🟡", "Faible": "🟢"}
    badge_cls_map = {"Critique": "critique", "Élevé": "eleve", "Modéré": "modere", "Faible": "faible"}
    for col, niveau in zip([col1, col2, col3, col4], ["Critique", "Élevé", "Modéré", "Faible"]):
        col.markdown(f"""
        <div class='kpi-card'>
            <div class='kpi-value' style='color:{RISK_COLORS[niveau]}'>{dist[niveau]}</div>
            <div class='kpi-label'>{icons[niveau]} {niveau}</div>
        </div>""", unsafe_allow_html=True)
    ...
    display_df.columns = ["Client ID", "Type", "Conso Moy (kWh)", "Écart-type", "Score (%)", "Niveau"]
    display_df["Conso Moy (kWh)"] = display_df["Conso Moy (kWh)"].round(2)
    display_df["Écart-type"] = display_df["Écart-type"].round(2)
...
# ══════════════════════════════════════════════
    ...
    st.markdown("## 📈 Prévision Énergétique")

    # Résultats modèles (fixes issus de vos expériences)
    model_results = pd.DataFrame({
        "Modèle":  ["Prophet (baseline)", "GRU", "CNN-LSTM", "LSTM (meilleur)"],
        "MAE (kWh)": [13.56, 5.32, 5.27, 4.23],
        "RMSE (kWh)": [16.19, 7.35, 7.00, 5.65],
        "Amélioration vs Prophet (%)": [0, 60.8, 61.1, 68.8],
    })

    st.markdown("<div class='section-title'>Comparaison des modèles de prévision</div>",
                unsafe_allow_html=True)

    col_m1, col_m2 = st.columns(2)
    with col_m1:
        fig_mae = px.bar(
            model_results, x="Modèle", y="MAE (kWh)", color="Modèle",
            color_discrete_sequence=["#FFD600", "#FF6B35", "#00E676", "#00C9FF"],
            title="MAE par modèle (↓ meilleur)", **PLOTLY_THEME,
        )
    ...
    with col_m2:
        fig_rmse = px.bar(
            model_results, x="Modèle", y="RMSE (kWh)", color="Modèle",
            color_discrete_sequence=["#FFD600", "#FF6B35", "#00E676", "#00C9FF"],
            title="RMSE par modèle (↓ meilleur)", **PLOTLY_THEME,
        )
        fig_rmse.update_layout(height=320, showlegend=False, yaxis=dict(gridcolor="#1E2535"))
        st.plotly_chart(fig_rmse, use_container_width=True)

    st.markdown("<div class='section-title'>Simulation de prévision — client sélectionné</div>",
                unsafe_allow_html=True)

    client_pred = st.selectbox("Client pour simulation", sorted(df["client_id"].unique()), key="pred_client")
    df_p = df[df["client_id"] == client_pred].sort_values("timestamp").tail(168)  # 7 jours

    # Simulation "prédiction" avec bruit gaussien (pas de vrai modèle ici)
    np.random.seed(42)
    ...
    fig_pred.add_trace(go.Scatter(x=df_p["timestamp"], y=df_p["consommation_kwh"],
                                  mode="lines", name="Réel", line=dict(color="#CCD6F6", width=2)))
    ...
    # Tableau récap
    st.markdown("<div class='section-title'>Récapitulatif des performances</div>", unsafe_allow_html=True)
    st.dataframe(model_results.set_index("Modèle"), use_container_width=True)

    st.info("💡 Le modèle **LSTM** atteint une amélioration de **+68.8%** par rapport à Prophet (baseline), "
...
# ══════════════════════════════════════════════
    ...
    eleves    = risk_df[risk_df["niveau_risque"] == "Élevé"]
    moderes   = risk_df[risk_df["niveau_risque"] == "Modéré"]
    ...
    if len(critiques):
        ...
        for _, row in critiques.iterrows():
            with st.expander(f"🔴 {row['client_id']} — Score {row['score_risque']:.1f}% — {row['type_client'].capitalize()}"):
                ...
                c2.metric("Écart-type", f"{row['conso_std']:.2f} kWh")
                c3.metric("Score Risque", f"{row['score_risque']:.1f}%", delta="⚠️ CRITIQUE")
                st.markdown("""
                **📋 Recommandations :**
                - 🔍 Inspection physique immédiate du compteur
                - 📞 Contacter le client sous 24h
                - 📊 Audit complet de la consommation sur 30 jours
                - 🔒 Surveillance renforcée (alertes temps réel)
                """)
        ...
        for _, row in eleves.iterrows():
            with st.expander(f"🟠 {row['client_id']} — Score {row['score_risque']:.1f}% — {row['type_client'].capitalize()}"):
                ...
                **📋 Recommandations :**
                ...
                - 📅 Planifier une visite de contrôle dans la semaine
                - 🔔 Activer les alertes automatiques
                """)

    # ── Clients modérés ──
    if len(moderes):
        st.markdown("### 🟡 Clients à Risque Modéré")
        with st.expander(f"Voir les {len(moderes)} clients à risque modéré"):
            st.dataframe(
    ...
    # ── Résumé ──
    st.markdown("---")
    st.markdown("### 📊 Synthèse des Anomalies Détectées")
    ...
    | Type de fraude | Détection |
    |---|---|
    | Pic artificiel (+200%) | **100%** ✅ |
    | Coupure simulée | **100%** ✅ |
    | Sous-comptage (-30%) | **38%** ⚠️ |
    """)
